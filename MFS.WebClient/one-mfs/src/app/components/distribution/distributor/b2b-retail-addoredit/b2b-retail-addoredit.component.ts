...
@Component({
  selector: 'app-b2b-retail-addoredit',
  templateUrl: './b2b-retail-addoredit.component.html',
  styleUrls: ['./b2b-retail-addoredit.component.css']
})
export class B2bRetailAddoreditComponent implements OnInit {
    ...
    saveB2bRetail(event): any {
        ...
        if (this.regInfoModel.distCode != "" || this.regInfoModel.branchName != "") {
            this.distributionService.SaveB2bRetal(this.regInfoModel, this.isEditMode, event).pipe(first())
                .subscribe(
                    data => {
                        if (data === 200) {
                            window.history.back();
                            if (this.isEditMode && !this.isRegistrationPermitted)
                                this.messageService.add({ severity: 'success', summary: 'Update successfully', sticky: true, detail: 'B2B Retail updated: ' + this.regInfoModel.mphone });
                            else if (this.isEditMode && this.isRegistrationPermitted)
                                this.messageService.add({ severity: 'success', summary: 'Register successfully', sticky: true, detail: 'B2B Retail registerd: ' + this.regInfoModel.mphone });
                            else {
                                this.messageService.add({ severity: 'success', summary: 'Save successfully', sticky: true, detail: 'B2B Retail added: ' + this.regInfoModel.mphone });
                            }
                        }
                        else {
