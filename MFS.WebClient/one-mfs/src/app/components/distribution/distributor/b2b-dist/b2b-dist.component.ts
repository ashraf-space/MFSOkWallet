...
@Component({
    ...
    templateUrl: './b2b-dist.component.html',
    styleUrls: ['./b2b-dist.component.css']
})
export class B2bDistAddoreditComponent implements OnInit {
    ...
    saveDSR(event): any {
        ...
        if (this.regInfoModel.distCode != "" || this.regInfoModel.branchName != "") {
            this.distributorService.saveB2bDistributor(this.regInfoModel, this.isEditMode, event).pipe(first())
                .subscribe(
                    data => {
                        if (data === 200) {
                            window.history.back();
                            if (this.isEditMode) {
                                ...
                                this.messageService.add({ severity: 'success', summary: 'Register successfully', sticky: true, detail: 'Distributor Registered: ' + this.regInfoModel.mphone });
                            }
                            else {
