    ...
    public class DistributorRepository : BaseRepository<Reginfo>, IDistributorRepository
    {
        ...
        public object GetRegionDetailsByMobileNo(string mobileNo)
        {
            try
            {
                using (var connection = this.GetConnection())
                {
                    string query = @"Select Dist_code as DistCode,(select Name from one.Location where code= SUBSTR(Dist_code, 1, 2)) as Region,
                                    (select Name from one.Location where code= SUBSTR(Dist_code, 1, 4)) as Area,
                                    (select Name from one.Location where code= SUBSTR(Dist_code, 1, 6)) as Territory from " + dbUser + "Reginfo where mphone = " + "'" + mobileNo + "'" + "";
                    var result = connection.Query<dynamic>(query).FirstOrDefault();
                    this.CloseConnection(connection);
                    return result;
                }
