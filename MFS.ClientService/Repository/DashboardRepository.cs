...
namespace MFS.ClientService.Repository
{
    public interface IDashboardRepository : IBaseRepository<DashboardViewModel>
    {
		...
		double GetutilityAmountByDate(DateTime fromDate,DateTime toDate,string utility);
	}
    ...
    public class DashboardRepository : BaseRepository<DashboardViewModel>, IDashboardRepository
    {
        ...
        public async Task<object> GetDataForDashboard()
        {
            try
            {
                ...
                count(year.mphone) as clientThisYear from " + dbUser + "reginfo r LEFT JOIN " + dbUser + "RegInfo dist on dist.Mphone = r.mphone and dist.Cat_ID='D' and dist.Status<>'C' LEFT JOIN " + dbUser + "RegInfo cust on cust.Mphone = r.mphone and cust.Cat_ID='C' and cust.Status<>'C'  LEFT JOIN " + dbUser + "RegInfo agent on agent.Mphone = r.mphone and agent.Cat_ID='A' and agent.Status<>'C'  LEFT JOIN " + dbUser + "RegInfo merchant on merchant.Mphone = r.mphone and merchant.Cat_ID='M' and merchant.Status<>'C'   LEFT JOIN (select r.mphone from " + dbUser + "reginfo r inner join " + dbUser + "merchant_config mc on r.mphone = mc.mphone where mc.category = 'E' and r.Status <> 'C') merOnline on merOnline.Mphone = r.mphone LEFT JOIN (select r.mphone from " + dbUser + "reginfo r inner join one.merchant_config mc on r.mphone = mc.mphone where mc.category in ('C','M') and r.Status<>'C') merOffline on merOffline.Mphone = r.mphone  LEFT JOIN " + dbUser + "RegInfo dsr on dsr.Mphone = r.mphone and dsr.Cat_ID='R' and dsr.Status <> 'C' LEFT JOIN " + dbUser + "REGINFO MMS ON MMS.Mphone = r.mphone and MMS.Cat_ID IN ('MMSC','MMSM') and MMS.Status <> 'C' LEFT JOIN " + dbUser + "REGINFO EMS ON EMS.Mphone = r.mphone and EMS.Cat_ID IN('EMSM','EMSC') and EMS.Status <> 'C' LEFT JOIN " + dbUser + "REGInFO mon on mon.mphone = r.mphone and to_char(mon.Autho_DATE,'MM') = to_CHAR(SYSDATE,'MM') and to_char(mon.Autho_DATE,'YYYY') = to_CHAR(SYSDATE,'YYYY') and mon.Status <> 'C' LEFT JOIN " + dbUser + "REGInFO year on year.mphone = r.mphone and to_char(year.Autho_date,'YYYY') = to_CHAR(SYSDATE,'YYYY') and year.Status <> 'C' where r.reg_Status = 'P'";
                ...
                from " + dbUser + "GL_TRANS_MST a where to_char(a.Trans_Date, 'YYYY') = to_char(sysdate, 'YYYY') group by to_char(a.Trans_Date, 'MM') ,to_char(a.Trans_Date, 'MON') order by to_char(a.Trans_Date, 'MM')";
                    ...
                    string barQuery = "Select ('Up to Dec ' || (EXTRACT(YEAR  FROM  SYSDATE)-1)) as Caption,count(agent.mphone) as Agent,count(cus.mphone) as Customer , count(dist.mphone) as distributor,count(merchant.mphone) as Merchant,count(merOnline.mphone) as MerchantOnline,count(merOffline.mphone) as MerchantOffline from " + dbUser + "reginfo r LEFT JOIN " + dbUser + "RegInfo cus on cus.Mphone = r.mphone and cus.Cat_ID = 'C'  and cus.Status<>'C'  LEFT JOIN " + dbUser + "RegInfo agent on agent.Mphone = r.mphone and agent.Cat_ID = 'A'  and agent.Status <>'C'  LEFT JOIN " + dbUser + "RegInfo dist on dist.Mphone = r.mphone and dist.Cat_ID = 'D' and dist.Status<>'C'   LEFT JOIN " + dbUser + "RegInfo merchant on merchant.Mphone = r.mphone and merchant.Cat_ID = 'M' and merchant.Status <>'C'      LEFT JOIN (select r.mphone from " + dbUser + "reginfo r inner join " + dbUser + "merchant_config mc on r.mphone = mc.mphone where mc.category = 'E') merOnline on merOnline.Mphone = r.mphone LEFT JOIN(select r.mphone from " + dbUser + "reginfo r inner join " + dbUser + "merchant_config mc on r.mphone = mc.mphone where mc.category in ('C','M')  and r.Status <>'C' ) merOffline on merOffline.Mphone = r.mphone  WHERE EXTRACT(YEAR FROM  r.reg_date) < EXTRACT(YEAR FROM  SYSDATE) and r.reg_Status = 'P'";
		...
		public double GetutilityAmountByDate(DateTime fromDate, DateTime toDate, string utility)
		{
			try
			{
				using (var connection = this.GetConnection())
				{
					...
					dyParam.Add("V_TOCATID", OracleDbType.Varchar2, ParameterDirection.Input, utility.ToUpper());
					dyParam.Add("CUR_DATA", OracleDbType.RefCursor, ParameterDirection.Output);

					var result = SqlMapper.Query<UtilityDashboard>(connection, dbUser + "SP_GET_UTILITY_AMOUNT", param: dyParam, commandType: CommandType.StoredProcedure).FirstOrDefault();
					this.CloseConnection(connection);
					
					return Convert.ToDouble(result.Amount);
				}
			}
			catch (Exception ex)
			{
