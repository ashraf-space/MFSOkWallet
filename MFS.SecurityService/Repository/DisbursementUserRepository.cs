...
{
    public interface IDisbursementUserRepository : IBaseRepository<DisbursementUser>
    {
        DisbursementUser validateLogin(string userName, string password);
    ...
    public class DisbursementUserRepository : BaseRepository<DisbursementUser>, IDisbursementUserRepository
    {
        //private static string dbUser;
        //public DisbursementUserRepository(MainDbUser objMainDbUser)
        //{
        //    dbUser = objMainDbUser.DbUser;
        //}
        MainDbUser mainDbUser = new MainDbUser();

        public DisbursementUser validateLogin(string userName, string password)
        {
            try
            {
                using (var conn = this.GetConnection())
                {
                    ...
                    IList<DisbursementUser> result = SqlMapper.Query<DisbursementUser>(conn, mainDbUser.DbUser + "PR_VALIDATELOGIN_DISBURSEMENT", param: dyParam, commandType: CommandType.StoredProcedure).ToList();
                    this.CloseConnection(conn);

                    if (result.Count == 0)
                    {
                        DisbursementUser obj = conn.QueryFirstOrDefault<DisbursementUser>("Select " + this.GetCamelCaseColumnList(new DisbursementUser()) + " from " + mainDbUser.DbUser + "MERCHANT_USER where mobile_no='" + userName + "'");
            ...
            {
                return new DisbursementUser() { Is_validated = false };
            }
        ...
        public object GetAllDisbursementUserList(string roleName)
        {
            try
            {
                using (var connection = this.GetConnection())
                {
                    ...
                    string query = @"Select c.company_name,u.username, u.mobile_no, u.log_in_status,u.id  from" + mainDbUser.DbUser + "Disbursement_user u inner join " + mainDbUser.DbUser + "tbl_disburse_company_info c on u.company_id = c.company_id";
                    //if ((roleName == "Admin") || (roleName == "System Admin") || (roleName == "Super Admin"))
                    //{
                    //    query = @"Select a.name, a.username,r.name as RoleName, a.mobile_no ,a.email_id,a.log_in_status,a.pstatus,a.id from" + mainDbUser.DbUser + "disbursement_user a inner join" + mainDbUser.DbUser + "role r on a.role_id=r.id";
                    //}
                    //else
                    //{
                    //    query = @"Select a.name, a.username,r.name as RoleName, a.mobile_no ,a.email_id,a.log_in_status,a.pstatus,a.id from" + mainDbUser.DbUser + "disbursement_user a inner join" + mainDbUser.DbUser + "role r on a.role_id=r.id where r.Name in ('Branch Teller','Branch KYC Maker','Branch KYC Checker')";
                    //}
        ...
        public object CheckDisbursementUserAlreadyExist(string username)
        {
            try
            {
                using (var connection = this.GetConnection())
                {
                    string query = @"select t.* from " + mainDbUser + "disbursement_user t where LOWER(t.username) = '" + username.ToLower() + "'";
                    var result = connection.Query<DisbursementUser>(query).FirstOrDefault();
                    this.CloseConnection(connection);
                    return result;
                }
