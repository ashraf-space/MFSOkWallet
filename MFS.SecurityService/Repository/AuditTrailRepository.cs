    ...
    public class AuditTrailRepository : BaseRepository<AuditTrail>, IAuditTrailRepository
    {
        ...
        public object GetTrailDtlById(string id)
        {
            try
            {
                using (var connection = this.GetConnection())
                {
                    string query = @"select t.which_field_name as ""fieldName"", t.which_value as ""preValue"", t.what_value as ""curValue"" from " + dbUser + "audit_trail_dtl t where t.audit_trail_id = '" + id + "'";
        ...
        public object GetUserListDdl()
        {
            try
            {
                using (var connection = this.GetConnection())
                {
                    string query = @"select t.username as ""label"", t.username ""value"" from " + dbUser + "application_user t";
        ...
        public object InsertIntoAuditTrail(AuditTrail model)
        {
            try
            {
                using (var conn = this.GetConnection())
                {
                    ...
                    SqlMapper.Query(conn, dbUser + "PROC_AUDIT_TRAIL_V2", param: dyParam, commandType: CommandType.StoredProcedure);
                    conn.Close();
